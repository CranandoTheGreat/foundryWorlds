import { gsap } from "../combatready.js";
import { getCanvas, getCombats, getGame } from "./settings.js";
import { availableAnimations, currentAnimation } from "./api.js";
...
export class CombatReadyAnimation extends SettingsAwareEntity {
    ...
    prepare() {
        if (!this.testMode) {
            ...
            //@ts-ignore
            let selectedToken = (getCanvas().tokens?.objects?.children.find(e => e._controlled))?.id ?? '';
            var testCombatant = new Combatant({ tokenId: selectedToken, name: (selectedToken == "") ? getGame().user?.name : undefined }, { parent: testCombat });
            let name = "";
...
export class NativeAnimation extends CombatReadyAnimation {
    ...
    initialize() {
        ...
        label.style.color = this.getSetting("fontcolor");
        // chevrons
        for (let idx = 0; idx < 6; ++idx) {
            ...
            chevron.style.color = this.getSetting("arrowscolor");
    ...
    onClickTurnBanner(ev) {
        document.removeEventListener("click", currentAnimation.onClickTurnBanner);
        currentAnimation.playAcknowledge();
        currentAnimation.cleanAnimations();
    }
    onClickNextBanner(ev) {
        document.removeEventListener("click", currentAnimation.onClickNextBanner);
        // kill next label anim if the user is fast
        let anims = gsap.getTweensOf(currentAnimation.LABEL);
        for (let tween of anims) {
            tween.kill();
        }
        // hide cover, but keep the beams to let the user know their turn is coming up!
        $(currentAnimation.BANNER).addClass("combatready-bannerdisable");
        gsap.to(currentAnimation.LABEL, 0.5, {
            opacity: 0.3,
            onComplete: function () {
                if (currentAnimation.getSetting("disablenextuplingering")) {
                    currentAnimation.BANNER.style.display = "none";
                }
            },
        });
        gsap.to(currentAnimation.COVER, 0.5, {
            opacity: 0,
            onComplete: function () {
                currentAnimation.COVER.style.display = "none";
            },
        });
    }
    cleanAnimations() {
    ...
    nextUpAnimation() {
        ...
        if (this.getSetting("animationstyle") !== "None") {
            ...
            if (this.getSetting("removeanimationsafter") < 0) {
            ...
            gsap.to(this.LABEL, 1, {
                delay: 2, opacity: 1,
                onComplete: function () {
                    if (currentAnimation.getSetting("removeanimationsafter") >= 0) {
                        setTimeout(() => { currentAnimation.cleanAnimations(); }, currentAnimation.getSetting("removeanimationsafter") * 1000);
                    }
                }
            });
        }
...
export class AnimationSubSettings extends CombatReadySubSettings {
    ...
    getData(options) {
        ...
        data.animations = Object.values(availableAnimations).map(iAnimation => {
            ...
            if (iAnimation.id == "native")
                animation.selectTitle = iAnimation.name;
