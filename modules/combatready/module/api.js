...
function setupAnimation(animation) {
    ...
    getGame().settings.settings.get(MODULE_NAME + ".selectedAnimation").default = getDefaultAnimation();
}
/**
...
function setupTimer(timer) {
    ...
    getGame().settings.settings.get(MODULE_NAME + ".selectedTimer").default = getDefaultTimer();
}
/**
...
export async function updateAnimation() {
    const selectedAnimation = getGame().settings.get(MODULE_NAME, "selectedAnimation");
    currentAnimation?.destroy();
    //@ts-ignore
    currentAnimation = availableAnimations[selectedAnimation] ?? availableAnimations[getGame().settings?.settings?.get(MODULE_NAME + ".selectedAnimation").default];
    //@ts-ignore
...
export async function updateTimer() {
    const selectedTimer = getGame().settings.get(MODULE_NAME, "selectedTimer");
    currentTimer?.destroy();
    //@ts-ignore
    currentTimer = availableTimers[selectedTimer] ?? availableTimers[getGame().settings?.settings?.get(MODULE_NAME + ".selectedTimer").default];
    //@ts-ignore
