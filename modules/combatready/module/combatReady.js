import { getCanvas, getCombats, getGame, MODULE_NAME } from "./settings.js";
...
export class CombatReady {
    ...
    static resolvePlayersPermission(permString, callback) {
        switch (permString) {
            ...
            case "OnlyPlayers":
                ...
                break;
            case "Player":
                if (((CombatReady.DATA.currentCombatant?.actor?.isOwner ?? false) && !getGame().user?.isGM)
    ...
    static playSound(sound) {
        let playTo = "Everyone";
        try {
            playTo = getGame().settings.get(MODULE_NAME, sound.setting);
        }
        catch (e) { }
    ...
    static init() {
        ...
        // sound statics
        CombatReady.TURN_SOUND = { file: getGame().settings.get(MODULE_NAME, "turnsoundfile"), setting: "turnsound" };
        CombatReady.NEXT_SOUND = { file: getGame().settings.get(MODULE_NAME, "nextsoundfile"), setting: "nextsound" };
        CombatReady.ROUND_SOUND = { file: getGame().settings.get(MODULE_NAME, "roundsoundfile"), setting: "roundsound" };
        CombatReady.EXPIRE_SOUND = { file: getGame().settings.get(MODULE_NAME, "expiresoundfile"), setting: "expiresound" };
        CombatReady.ACK_SOUND = { file: getGame().settings.get(MODULE_NAME, "acksoundfile"), setting: "acksound" };
        CombatReady.TICK_SOUND = { file: getGame().settings.get(MODULE_NAME, "ticksoundfile"), setting: "ticksound" };
    }
    /**
     * Animate... Weee
     */
    ...
    static runAnimationForPlayer(combatant) {
    ...
    static async checkForCurrentTurnAnimation() {
        ...
        let panToTokenPerm = getGame().settings.get(MODULE_NAME, "pantotoken");
        if (runAnimation) {
            ...
            if (getGame().settings.get(MODULE_NAME, "endturndialog"))
    ...
    static async checkForNextTurnAnimation() {
        ...
        if (runAnimation) {
            if (nextTurn == 0 && getGame().settings.get(MODULE_NAME, "disablenextuponlastturn"))
                return false;
    ...
    static async toggleCheck(newRound = false) {
        ...
        if (CombatReady.DATA.currentCombat && CombatReady.DATA.currentCombat.started) {
            if (getGame().settings.get(MODULE_NAME, "wrapitupdialog")) {
    ...
    static nextRound() {
        let roundSoundPerm = getGame().settings.get(MODULE_NAME, "roundsound");
    ...
    static async timerTick(TIMECURRENT = null) {
        ...
        if (CombatReady.TIMEMAX - CombatReady.TIMECURRENT <= getGame().settings.get(MODULE_NAME, "tickonlast")) {
        ...
        if (width > 100) {
            ...
            if (getGame().settings.get(MODULE_NAME, "autoendontimer")) {
